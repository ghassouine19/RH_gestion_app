...
import com.example.rhappback.service.PdfService;
import com.example.rhappback.service.UserService;
...
public class UserController {
    ...
    @GetMapping(value = "/{id}/work-certificate", produces = MediaType.APPLICATION_PDF_VALUE)
    public ResponseEntity<InputStreamResource> generateWorkCertificate(@PathVariable Long id) {
        User user = userRepository.findById(id).orElseThrow(() -> new RuntimeException("User not found"));
        ByteArrayInputStream bis = pdfService.generateWorkCertificate(user);

        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Disposition", "inline; filename=work_certificate_" + id + ".pdf");
